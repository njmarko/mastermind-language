...
// Ubacuje konstantu u tabelu simbola (ako vec ne postoji) na odredjenom nivou
// u atr2 se cuva blok
int insert_literal_block(char *str, unsigned type, int atr2) {
  int idx;
  for(idx = first_empty - 1; idx > FUN_REG; idx--) {
    if(strcmp(symbol_table[idx].name, str) == 0 
       && symbol_table[idx].type == type && symbol_table[idx].atr2 == atr2)
       return idx;
  }
  ...
  if(((type==INT) && (num<INT_MIN || num>INT_MAX) )
    || ((type==UINT) && (num<0 || num>UINT_MAX)) )  
      err("literal out of range");
  idx = insert_symbol(str, LIT, type, NO_ATR, atr2);
  return idx;
}


// Vraca indeks pronadjenog simbola kojem se i atr2 poklapa ili vraca -1. Moze se koristiti za rad sa blokovima
int lookup_symbol_block(char *name, unsigned kind, int atr2){
  int i;
  for(i = first_empty - 1; i > FUN_REG; i--) {
    if(strcmp(symbol_table[i].name, name) == 0 
       && symbol_table[i].kind & kind && symbol_table[i].atr2 == atr2)
       return i;
  }
  return -1;
}

// Vraca indeks pronadjenog simbola kojem se i tip i atr2 poklapa ili vraca -1. Moze se koristiti za rad sa ugnjezenim check iskazima
int lookup_symbol_type_block(char *name, unsigned kind, unsigned type, int atr2){
  int i;
  for(i = first_empty - 1; i > FUN_REG; i--) {
    if(strcmp(symbol_table[i].name, name) == 0 
       && symbol_table[i].kind & kind && symbol_table[i].type == type && symbol_table[i].atr2 == atr2)
       return i;
  }
  return -1;
}
